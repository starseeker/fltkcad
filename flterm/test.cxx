...
//printf("(%d, %d):%s\n", x, y, string);
	...
	if (flags & BOLD) 
	    XDrawString(display, window, gc, x*font_width+1, y*font_height+font_info->ascent, (char *)string, len);
...
int main(void)
{
	...
	XTerm term;
	fd_set rset;
	int mfd, xfd, high, i, ret, need_timeout = 1;
	struct timeval tv;
	...
	term.clear_mode_flag(GTerm::LOCALECHO);
	...
	while (i >= 0) 
	{
		...
		ret = select(high+1, &rset, NULL, NULL, need_timeout ? (&tv) : NULL);
		if (!ret)
		{	/* timer expired */
			term.Update();
			need_timeout = 0;
